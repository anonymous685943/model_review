...
def run_experiment(
    ...
    window_size = [50, 150, 1050]  #  Window size in px for striding window
    border_distance = 0.5  # Min dist from to image to window border. 2=whole image size

    # Create a directory for the results
    date_name = datetime.datetime.now().strftime("%Y%b%d_%H:%M:%S")
    ...
    if exp_type == 'number_estimation':
        ...
        json_file_name = file_name_basis + 'results.json'
