...
def estimate_numbers(N,
    ...
    for i in range(len(iconic_memory)):
        img = create_image(iconic_memory[i], size, border_distance, minimum_distance, 'uniform', transform, radius, radius_distribution, radius_matrix, use_icon)
        ...
        for n in rangy:
            ...
            for item in memory_images:
                ...
                for window_size_i in window_size_list:
                    ...
                    similarity += window_size_i*greater_smaller_measure_i
                similarity /= sum(window_size_list)
